...
public class MatrixTests:GlWindow {
    ...
    const float PointPosition = .5f;
    static readonly Vector4[] lineSegments = {
        new(-PointPosition, -PointPosition, 0, 1),
        new(PointPosition, -PointPosition, 0, 1),
        new(PointPosition, PointPosition, 0, 1),
        new(-PointPosition, -PointPosition, 0, 1),
        new(PointPosition, PointPosition, 0, 1),
    ...
    static void Update0 (double dt, ref Vector4 p, ref Quaternion q) {
        q = Quaternion.Concatenate(q, Quaternion.CreateFromAxisAngle(Vector3.UnitZ, (float)(dt * Maths.dTau / 8)));
        var v = Vector3.Transform(Vector3.UnitY, q);
        p += new Vector4((float)dt * v, 0);
    }

    static void Update1 (double dt, ref Vector4 p, ref Quaternion q) {
        q = Quaternion.Concatenate(q, Quaternion.CreateFromAxisAngle(Vector3.UnitZ, (float)(dt * Maths.dTau / 8)));
    }

    Vector4 pos0 = new(1, 1, -1, 1);
    Vector4 pos1 = new(1, 1, -1, 1);
    Quaternion ori0 = Quaternion.Identity;
    Quaternion ori1 = Quaternion.Identity;

    protected override void Render () {
        Update0(LastFramesInterval, ref pos0, ref ori0);
        Update1(LastFramesInterval, ref pos1, ref ori1);
        var size = ClientSize;
